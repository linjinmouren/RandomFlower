...
public class BloodyEnchantressBlockEntity extends GeneratingFlowerBlockEntity {
    ...
    public void tickFlower() {
        ...
        int[] ampNum= ampArr.stream().map(entity -> entity.hasEffect(ModEffects.BLOOD_TEMPTATION.get()) ? Objects.requireNonNull(entity.getEffect(ModEffects.BLOOD_TEMPTATION.get())).getAmplifier() : 0).mapToInt(Integer::intValue).toArray();
        ...
        for (int i = 0; i < ampNum.length; i++) {
            if (getMana() < getMaxMana()) {
                if (Math.random() > 0.5F) {
                    ...
                    living.addEffect(new MobEffectInstance(ModEffects.BLOOD_TEMPTATION.get(), 8 * 20, amp + 1));
                    if (living instanceof ServerPlayer) AdvancementHandler.INSTANCE.grantAdvancement((ServerPlayer) living, Utils.BLOODY_ENCHANTRESS_USE);
